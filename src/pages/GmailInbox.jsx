...
export default function GmailInbox() {
  ...
  const safeCases = (cases || []).filter(c => c.channel === 'email')
    ...
    <div className="inbox-page">
      <div className="inbox-header">
        <div>
          <h1>Gmail inbox</h1>
          <p className="muted">
            {safeCases.length} of {safeCases.length} emails · {unreadCount} with unread
          </p>
        </div>
      ...
      <div className="inbox-table">
        ...
        {filtered.length === 0 && <div className="inbox-empty">No emails match your search.</div>}
      </div>
    </div>
  )
}
